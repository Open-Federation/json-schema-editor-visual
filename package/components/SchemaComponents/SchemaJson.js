      ...
      <div>
        <Row type="flex" justify="space-around" align="middle">
          ...
          >
            ...
            {value.$ref != null ? (
              <LoadRefComponent prefix={prefix} name={name} value={value} />
            ) : (
              ''
            )}
          </Col>
...
class SchemaObjectComponent extends Component {
  ...
  render() {
    ...
    console.log(
      _.findIndex(prefix, (x) => x === 'userUpdateProfileFromWechatMp'),
      'ssss'
    )
    if (
      _.findIndex(prefix, (x) => x === 'userUpdateProfileFromWechatMp') > -1
    ) {
      console.log(data)
...
const LoadRefComponent = (props, context) => {
  const { prefix, name, add, value } = props
  const Model = context.Model.schema
  console.log(value, 'LoadRefComponent value')
  return (
    <span
      onClick={() => {
        Model.setOpenValueAction({
